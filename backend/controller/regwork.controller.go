...
func GetPost(c *gin.Context) {
	...
	if err := entity.DB().Raw("SELECT * FROM posts WHERE id >= ?", id).Find(&post).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func SearchWork(c *gin.Context) {
    var search_w []entity.Post
    key := c.Param("key")

    // Corrected SQL query with placeholders
    query := "SELECT * FROM posts WHERE description LIKE ? OR position LIKE ? OR company_name LIKE ? LIMIT 100 OFFSET 0"
