...
function LoginOparator() {
  // const navigate = useNavigate();
  const [messageApi, contextHolder] = message.useMessage();
  const [oparatorid, setOparatorID] = useState();
  ...
  const onFinish = async (values: OparatorsInterface) => {
    ...
    try {
      const response = await OparatorLogin(values);
      setOparatorID(response.id);
      console.log(response);

      if ('accessToken' in response) {
        localStorage.setItem('accessToken', response.accessToken);
        // localStorage.setItem('oparator', JSON.stringify(response.oparator));
            ...
            <Card style={{ height: "130px",marginTop: '0px', marginBottom: "-30px", }}>
              <div className="label" style={{ marginLeft: "30px", marginRight: "30px" }}>
                <p className="div">
                  ...
                  <Button className="custom-button" danger>เข้าสู่ระบบ สำหรับผู้หางาน</Button>
                </p>
              </div>
            </Card>
            <Form
              ...
              <Card size="small" style={{ height: "220px" }}>
                <div style={{ marginBottom: "10px", marginTop: "20px", marginLeft: "30px", marginRight: "30px" }}>
                  <Col xs={24} sm={24} md={24} lg={24} xl={24}>
                    ...
                    >
                      <Input
                      // onChange={e => setOparator_Email(e.target.value)} 
                      />
                    </Form.Item>
                  </Col>
                  <Col xs={24} sm={24} md={24} lg={24} xl={24}>
